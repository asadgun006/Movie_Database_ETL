...
for name in info_box_list:
    if ('Budget' in name) & ('Box office' in name):
        name['Budget'] = convert_to_numeric(name['Budget'])
        name['Box office'] = convert_to_numeric((name['Box office']))
    elif 'Budget' in name:
        name['Budget'] = convert_to_numeric(name['Budget'])
        name['Box office'] = 'N/A'
...
for name in info_box_list:
    print(name)
with open("disney_data_refreshed.json", "w", encoding="utf-8") as f:
    json.dump(info_box_list, f, ensure_ascii=False, indent=1)
